...
@Component({
  selector: 'pending-products-list',
  templateUrl: './pending-products-list.component.html',
  styleUrls: ['./pending-products-list.component.scss']
})
export class PendingProductListComponent implements OnInit {
  ...
  productlist(page: number, pageSize: number) {
    this.productservice.allPendingProductlist(page + 1, pageSize).subscribe(
      res => {
