import { OrderActionBtnComponent } from './../order-action-btn/order-action-btn.component';
...
@Component({
  selector: 'app-return-list-vendor',
  templateUrl: './return-list-vendor.component.html',
  styleUrl: './return-list-vendor.component.scss',
  providers: [DatePipe, CurrencyPipe, TitleCasePipe]
})
export class ReturnListVendorComponent implements OnInit {
  ...
  orderList(page: number, pageSize: number) {
    console.log("Inside Return Order List Vendor Component");
    this.orderservice.returnOrderList(page + 1, pageSize).subscribe(
      res => {
