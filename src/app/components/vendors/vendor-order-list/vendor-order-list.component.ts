...
import { OrderActionBtnComponent } from '../../order-management/order-action-btn/order-action-btn.component';
...
export class VendorOrderListComponent implements OnInit {
  ...
  constructor(private vendorservice: VendorsService, private ngZone: NgZone, private router: Router, private route: ActivatedRoute, private datePipe: DatePipe, private currencypipe: CurrencyPipe, private titlecase: TitleCasePipe, private toastrService: ToastrService) {
  ...
  ngOnInit() {
    localStorage.setItem('vendor_id_order', this.route.snapshot.paramMap.get('id'));
  ...
  orderList(page: number, pageSize: number) {
    ...
    this.vendorservice.allOrderList(uoData).subscribe(
      res => {
  ...
  triggerSearch(filterValue: string): void {
    this.vendorservice.searchOrderList(filterValue, this.route.snapshot.paramMap.get('id')).subscribe(res => {
      if (res && res['data']) {
