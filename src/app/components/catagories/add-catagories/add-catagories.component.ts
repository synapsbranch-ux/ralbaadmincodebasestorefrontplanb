...
@Component({
  ...
  styleUrls: ['./add-catagories.component.scss']
})
...
export class AddCatagoriesComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer, private modalService: NgbModal, private formBuilder: FormBuilder, public catagoriesservice: CatagoriesService, private router: Router, private toastrService: ToastrService, private ngZone: NgZone) {
  }
  ...
  onCatagoriesSubmit() {
    if (this.catagoriesSubmitForm.invalid) {
      this.catagoriesSubmitForm.markAllAsTouched();
      return;
    }
    ...
    this.catagoriesservice.addCatagories(jsondata).subscribe(
      res => {
        this.catagoriesinsertMessage = "Category added successfully";
    ...
    reader.onload = (e: any) => {
      ...
      if (type) {
        ...
        if (att_temp_arr[0].hasOwnProperty('attribute_input') && att_temp_arr[0].hasOwnProperty('attribute_name') && att_temp_arr[0].hasOwnProperty('attribute_value') && att_temp_arr[0].hasOwnProperty('is_mandatory')) {
          ...
          this.toastrService.error('Please Upload a Valid Attribute JSON')
        }
